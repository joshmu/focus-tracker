...
let db = config.resetDb
  ? {}
  : JSON.parse(fs.readFileSync(__dirname + config.dbPath))
// console.log(config)
// console.log(db)
...
const getFocus = async () => {
  const result = await runJxa(
    (unicorn, horse) => {
      ...
      let appName = Application('System Events')
...
const run = async () => {
  ...
  if (sleepTxt.indexOf('"CurrentPowerState"=4') > -1) {
    ...
    console.log(`${app}: ${(db[app] / 60).toFixed(1)} minutes   (${db[app]})`)
